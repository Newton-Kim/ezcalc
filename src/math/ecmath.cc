...
inline void mathf_1arg(ezGC &gc, vector<ezValue *> &args,
                       vector<ezValue *> &rets,
  ...
  default:
    ...
    rets.push_back(
        (ezValue *)gc.add((ezGCObject *)new ezString("invalid argument type")));
    break;
  }
}
...
public:
  void run(ezGC &gc, vector<ezValue *> &args, vector<ezValue *> &rets) {
    ...
    case EZ_VALUE_TYPE_INTEGER: {
      ...
      rets.push_back(
          (ezValue *)gc.add((ezGCObject *)new ezInteger(v > 0 ? v : -v)));
    } break;
    case EZ_VALUE_TYPE_FLOAT: {
      double v = args[0]->to_float();
      rets.push_back(
          (ezValue *)gc.add((ezGCObject *)new ezFloat(v > 0 ? v : -v)));
    } break;
    case EZ_VALUE_TYPE_COMPLEX: {
    ...
    default:
      rets.push_back(ezNull::instance());
      rets.push_back((ezValue *)gc.add(
          (ezGCObject *)new ezString("invalid argument type")));
      break;
    }
  }
};
  ...
  void run(ezGC &gc, vector<ezValue *> &args, vector<ezValue *> &rets) {
    ...
    case EZ_VALUE_TYPE_INTEGER: {
      ...
      if (args[1]->type == EZ_VALUE_TYPE_INTEGER ||
        ...
        rets.push_back(
            (ezValue *)gc.add((ezGCObject *)new ezInteger(pow(v, e))));
      } else {
        rets.push_back(ezNull::instance());
        rets.push_back((ezValue *)gc.add(
            (ezGCObject *)new ezString("invalid exponent type")));
      }
    } break;
    case EZ_VALUE_TYPE_FLOAT: {
      double v = args[0]->to_float();
      if (args[0] == m_e || v == m_e->to_float()) {
        ...
        case EZ_VALUE_TYPE_FLOAT:
          rets.push_back((ezValue *)gc.add(
              (ezGCObject *)new ezComplex(exp(args[1]->to_float()))));
          break;
        case EZ_VALUE_TYPE_COMPLEX:
          rets.push_back((ezValue *)gc.add(
              (ezGCObject *)new ezComplex(exp(args[1]->to_complex()))));
          break;
        default:
          rets.push_back(ezNull::instance());
          rets.push_back((ezValue *)gc.add(
